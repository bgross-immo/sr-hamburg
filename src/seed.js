...
const runs = [
  { slug:'run-0', number:'Run 0', title:'Die undichte Stelle', date_played:'Vor dem Orkan „Njord“', date_start:'2026-01-08', tod_start:'abends', date_end:'2026-01-08', tod_end:'nachts',
  ...
  { slug:'run-1', number:'Run 1', title:'Das Auge des Sturms', date_played:'Orkan „Njord“, Nacht', date_start:'2026-01-08', tod_start:'nachts', date_end:'2026-01-08', tod_end:'nachts',
  ...
  { slug:'run-2', number:'Run 2', title:'Kein Ort zum Heilen', date_played:'Der Morgen nach dem Sturm', date_start:'2026-01-09', tod_start:'morgens', date_end:'2026-01-09', tod_end:'mittags',
...
const forceRun = db.prepare(`UPDATE runs SET title=?, number=?, date_played=?, participants=?, location=?, time_from=?, time_to=?, karma=?, nuyen=?, loot=?, new_connections=?, involved_connections=?, actors=?, summary=?, date_start=?, tod_start=?, date_end=?, tod_end=? WHERE slug=?`);
...
for (const r of runs) {
  ...
  for (const fld of ['date_start','tod_start','date_end','tod_end']) if (r[fld]) db.prepare(`UPDATE runs SET ${fld}=? WHERE slug=? AND (${fld} IS NULL OR ${fld}='')`).run(r[fld], r.slug);
  ...
  if ((r.owner || 'benjamin') === 'benjamin')
    forceRun.run(r.title||'', r.number||'', r.date_played||'', r.participants||'', r.location||'', r.time_from||'', r.time_to||'', r.karma||'', r.nuyen||'', r.loot||'', r.new_connections||'', r.involved_connections||'', r.actors||'', r.summary||'', r.date_start||'', r.tod_start||'', r.date_end||'', r.tod_end||'', r.slug);
}
